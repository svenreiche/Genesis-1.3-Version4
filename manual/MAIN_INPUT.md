...
- `seed` (*int, 123456789*): Seed to initialize the random number generator, which is used for shot noise calculation and undulator lattice errors, though it is recommended that the random number generator seed is redefined explicitly for undulator errors in its corresponding namelist. Each slice draws from its own sequence, keyed on the position of the slice in the time window, so a given seed produces the same shot-noise realization however many cores the run uses. Note that the time window itself can still depend on the number of cores; see the `time` namelist.
...
Genesis reports this at the start of the run whenever it happens. It is the only remaining
respect in which the number of cores affects the result: the shot noise and the other random
sequences are keyed on the position of a slice in the time window rather than on the core
which holds it, so two runs which agree on the time window agree on everything. Choosing
`SLEN` so that `SLEN / LAMBDA0 / SAMPLE` is a multiple of the number of cores avoids the
