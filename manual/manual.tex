...
\vspace{1cm}
{\rule{0cm}{2cm}\LARGE\sf\hspace{2cm}Version 4.0.2 - beta}\\
\vspace{1cm}
...
{\tt genesis4} {\it filename}

Any further information must be provided within the input deck. Starting Genesis in this way will effectively invoke a parallel job with only one single core. To start the code with just more than one core the launching command {\tt mpirun} needs to be called, including the number of cores. In its simplest call it would be:

{\tt mpirun -np \#\# genesis4} {\it filename}
...
This namelist is used to change the raw lattice from the lattice file, such as generating errors in the position of the elements. At the moment almost nothing has been implemented but will be implemented in the future
...
\item {\bf k1}: Normalized focusing strength in 1/m. Default  value is 0
